...
def test_layout_zones_and_neutrals() -> None:
    """Given 合成地图，When 查蓝/黄区与固定中立，Then 是「距基地块 1 环蓝 / 2 环黄」。

    旧实现把挑战者蓝区写成矩形 `x10..13,y20..23`、黄区 `x10..15,y17..19`，与任务书
    配图（基地 2×2 绿块，紧贴一圈蓝、再外一圈黄）不符；本测试按环断言，矩形化会立刻变红。
    """
    layout = make_layout()
    assert layout.base_cells("challenger") == frozenset(
        {(10, 24), (11, 24), (10, 25), (11, 25)}
    )
    assert layout.base_cells("defender") == frozenset(
        {(30, 10), (31, 10), (30, 11), (31, 11)}
    )
    # 蓝区是距离 1 环：基地正下方紧邻的 (10,26) 与正上方 (10,23) 都在内
    assert layout.in_weapon_zone(10, 26) and layout.in_weapon_zone(10, 23)
    assert not layout.in_weapon_zone(10, 27) and not layout.in_weapon_zone(10, 17)
    # 黄区是距离 2 环：再外一格的 (10,27) 在内；(10,17)（旧矩形里曾是黄区）已在外
    assert layout.in_wall_zone(10, 27) and not layout.in_wall_zone(10, 17)
    assert not layout.in_wall_zone(10, 26) and not layout.in_wall_zone(10, 23)
    # 双方各 12 格蓝环 / 20 格黄环；矩形化（16/24 格）会在这里变红
    assert len(layout.weapon_zone) == 24 and len(layout.wall_zone) == 40
    assert not (layout.weapon_zone & layout.wall_zone)
    assert not (set(layout.fixed_neutrals) & (layout.weapon_zone | layout.wall_zone))
    assert layout.fixed_neutrals[(20, 16)] == "vendor"
    assert layout.fixed_neutrals[(25, 20)] == "weaponShop"


def test_role_spawns_inside_blue_zone() -> None:
    """Given 合成地图，When 查 6 个角色出生格，Then 都在自家蓝区且不压基地。"""
    from future_war.sim.layout import ROLE_SPAWNS

    layout = make_layout()
    for team in ("challenger", "defender"):
        for role, cell in ROLE_SPAWNS[team].items():
            assert cell in layout.weapon_zone, f"{team}/{role} 出生格 {cell} 不在蓝区"
            assert cell not in layout.base_cells(team)
...
def test_build_weapon_wrong_zone_invalid() -> None:
    ...
    day_round(world)
    worker = world.teams["challenger"].units[10010]
    worker.x, worker.y = 10, 28
    results = resolve(
        world,
        challenger={10010: cmd("build", name="railgun", targetPos=(Pos(10, 27),))},
    )
    assert results["challenger"][10010] is False
...
def test_build_wall_consumes_stone() -> None:
    ...
    results = resolve(
        world,
        challenger={10010: cmd("build", name="wall", targetPos=(Pos(10, 27),))},
    )
    assert results["challenger"][10010] is True
    assert worker.backpack == ["stone"]
