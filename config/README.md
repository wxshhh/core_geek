...
| `build.wall_first_until_round` | int | `40` | **墙优先只占白天前段**（配合 `build.wall_first`）：白天第 `< 该值` 回合保持「全队采石 → 砌墙」；第 `>= 该值` 回合退出墙优先、回到普通经济（按 `_plan_sell`/`economy.sell_batch` 采铜/铁成批卖给小贩，把金币攒起来买券）。线上 issue #2 的根因就是全天墙优先：全队一直采石 → 没人采铜铁 → 卖不出钱 → 连 20 金围墙券都买不起 → 武器/围墙永远 L1 → 夜里成批阵亡。**墙已建满或黄区无候选时不论回合立即退出**（`_wall_done`）；退出后**一律不再主动跑去采石**（`_needs_stone` 在此窗口外直接返回否，不是因为「石头已够」而是「这个理由不成立」），矿石优先级回到 copper > iron > stone 的变现口径 —— 否则全队石头只要低于 `economy.stone_reserve`（默认 4）就又被拽回石矿，调小 `stone_reserve` 也救不了（模拟器实测：D1 末 gold 1、walls 8）。退出只影响「要不要**主动**采石」，**顺路砌墙（相邻即建）与破口/缺口重建照旧**；次日白天第 1 回合 `_day_round` 归零、窗口按天重开，所以「墙没建满但已过 40 回合」不会永久断料。调大 = 更偏防守、金币回流更晚；调小 = 更早转收入 |
