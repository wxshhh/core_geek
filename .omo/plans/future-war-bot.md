...
- [x] F1. 方案符合性审计：逐条核对实现是否覆盖 Scope 的 Must have、是否违反 Must NOT have。
- [x] F2. 代码质量评审：结构、可读性、无 AI slop、无 `except` 吞异常、类型清晰。
- [x] F3. 真实手动 QA：在模拟器中完整跑一场多天对局，验证存活、任务、寻宝、进攻链路，并确认**日志与摘要可直接支撑一句话反馈**。
- [x] F4. 范围保真：确认未引入 Scope OUT 内容（RL/外网/硬编码/改判题器）。
